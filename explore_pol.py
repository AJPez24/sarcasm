...
with open("comments-pol.json", "r") as f:
...
# save new version
comments_df.to_csv("comments-pol-clean.csv")

# load train csv
train_raw = pd.read_csv("train-balanced.csv", header=None, names=["raw"])
...
author_stats['sarcasm_ratio'] = author_stats['sarcasm_count'] / author_stats['total_comments']
...
plt.figure(figsize=(12,6))
plt.bar(top_authors.index, top_authors['sarcasm_ratio'], color='blue')
plt.xticks(rotation=45, ha='right')
plt.title("Sarcasm Ratio for Top 10 Authors")
plt.ylabel("Sarcasm Ratio")
plt.xlabel("Author")
plt.tight_layout()
plt.show()